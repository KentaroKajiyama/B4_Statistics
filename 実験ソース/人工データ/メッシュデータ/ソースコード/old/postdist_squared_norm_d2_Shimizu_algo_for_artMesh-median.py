...
def main():
    ...
    # ディレクトリの指定 実験データ/人口データ/１乗
    experimentPath = Path(__file__).resolve().parent.parent.parent.parent.parent.joinpath("実験データ/人工データ/１乗")
    # 現在の日時を取得
    now = datetime.now()
    # 日時を文字列としてフォーマット
                ...
                for j in range(len(coords_population)):
                    ...
                    if boolean_point_in_polygon(sample_point_judge, polygon):
                        ...
                        tmp_sigma_upper += coords_population[j][2]*(dist(
                            coords_population[j][0], coords_population[j][1], pnts[i][0], pnts[i][1]))
