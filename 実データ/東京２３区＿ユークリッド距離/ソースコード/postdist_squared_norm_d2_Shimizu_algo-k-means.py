...
def main():
    ...
    # 初期状態のコストを計算
    cost = k_means.cost_function(coords_population[:,:2],coords_population[:,2:].ravel(),pnts, non_claster = True)
    ...
    MaxIterations = 100
    # 実行
    optimized_pnts, labels, cost = k_means.weighted_kmeans(coords_population[:,:2],coords_population[:,2:].ravel(), n, pnts = pnts, max_iter = MaxIterations, initial = True)
    # 解の描画
