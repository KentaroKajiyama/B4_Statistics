...
def main():
    ...
    while 1 : 
        ...
        if norm(g,pnts,eps):
            pnts = g
            break
        elif while_counter == 20:
            pnts = g
            print("while counter = 20")
            break
    ...
    for vor_poly_num in range(vor_poly_counter_box[vor_poly_counter]):
        ...
        for j in range(len(coords_population)):
            ...
            if boolean_point_in_polygon(sample_point_judge, polygon):
                ...
                tmp_sigma_upper_x += coords_population[j][2]*coords_population[j][0]
                tmp_sigma_upper_y += coords_population[j][2]*coords_population[j][1]
