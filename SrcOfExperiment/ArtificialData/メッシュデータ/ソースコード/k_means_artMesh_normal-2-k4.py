...
# 初期点を指定する場合
ISRANDOM = True
POINTS = np.array([[-1.96,0],[1.96,0],[0,0.91],[0,-0.91]])
################################################################

def main(i,MeshNumber=0,coords_population=None, xx=None, yy=None, ww=None,CreatedMesh = False, mu = None, sigma = None):
    # ディレクトリの指定 実験データ/人口データ/ランダム/２乗/case2
    experimentPathParent = Path(__file__).resolve().parent.parent.parent.parent.parent.joinpath("実験データ/人工データ/メッシュ/正規分布/２乗/case2")
    # 現在の日時を取得
    now = datetime.now()
    # 日時を文字列としてフォーマット
    formatted_now = now.strftime("%Y-%m-%d %H:%M:%S")
    # 保存用ディレクトリの指定
    experimentPath = experimentPathParent.joinpath(formatted_now+"_"+str(i+1)+"k4")
    # 保存用ディレクトリの作成
    os.mkdir(experimentPath) 
    # 結果の保存先
    resultfile = "result_artMesh_Mean_normal_case2_k4.csv"
    ...
    with open(experimentPathParent.joinpath("cost_stock_k4.csv"), "a") as f:
        np.savetxt(f, [optimized_cost], fmt = '%f')
    return 0
